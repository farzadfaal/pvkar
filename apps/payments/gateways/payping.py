...
from config.settings.base import PAYPING_API_KEY
...
def confirmpay(amount, refId, token):
    ...
    url = PAYPING_PAY_URL
    headers = {"Authorization": f"Bearer {token}", "Content-type": "application/json"}
    postobject = {"amount": amount, "refId": refId}

    # Send Confirm Payment Request
    try:
        res = requests.post(
            url + "/verify", data=json.dumps(postobject), headers=headers
        )
    except requests.exceptions.RequestException as e:
        raise SystemExit(e)
